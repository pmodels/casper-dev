                                 ...
                                 MPI_Request * request)
{
    ...
    CSP_DBG_PRINT("CASPER Rput to target %d, target_disp=0x%lx, target_count=%d, num_segs=%d\n",
    ...
    for (i = 0; i < num_segs; i++) {
        ...
        mpi_errno = PMPI_Rput(decoded_ops[i].origin_addr, decoded_ops[i].origin_count,
        ...
        CSP_DBG_PRINT("CASPER Rput to (ghost %d, win 0x%x) instead of "
                      "target %d, seg %d \n"
                         ...
                         CSP_win * ug_win, MPI_Request * request)
{
    ...
    if (CSP_put_async_disabled(target, ug_win)) {
        mpi_errno = PMPI_Rput(origin_addr, origin_count, origin_datatype,
                              target->ug_rank, target_disp, target_count, target_datatype, *win_ptr,
                              request);

        CSP_ADAPT_PROF_INC_TO_USER_CNT(rank, CSP_ADPT_PROF_PUT);
    ...
    if (target_rank == rank && ug_win->is_self_locked) {
        ...
        mpi_errno = CSP_rput_shared_impl(origin_addr, origin_count, origin_datatype, target_rank,
                                         target_disp, target_count, target_datatype,
                                         ug_win, request);
        if (mpi_errno != MPI_SUCCESS)
            return mpi_errno;
...
#endif
    {
        ...
        if (CSP_ENV.lock_binding == CSP_LOCK_BINDING_SEGMENT &&
            ...
            mpi_errno = CSP_rput_segment_impl(origin_addr, origin_count, origin_datatype,
             ...
             int target_count, MPI_Datatype target_datatype, MPI_Win win, MPI_Request * request)
{
    ...
    if (ug_win) {
        /* casper window */
        mpi_errno = CSP_rput_impl(origin_addr, origin_count, origin_datatype, target_rank,
                                  target_disp, target_count, target_datatype, ug_win, request);
    }
    else {
        /* normal window */
        mpi_errno = PMPI_Rput(origin_addr, origin_count, origin_datatype, target_rank,
