...
int CSP_topo_remap(void)
{
    ...
    MPI_Group old_local_group = MPI_GROUP_NULL, old_wgroup = MPI_GROUP_NULL;
    ...
    if (global_remap_flag) {
        ...
        old_local_group = CSP_PROC.lgroup;
  ...
  fn_exit:
    ...
    if (old_local_group != MPI_GROUP_NULL)
        CSP_CALLMPI_EXIT(PMPI_Group_free(&old_local_group));
