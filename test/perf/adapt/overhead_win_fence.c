...
/* This benchmark measures the overhead of win_fence with different
 * adaptation modes. */
...
static void run_test(const char *config_val)
{
    ...
    t1 = MPI_Wtime();
    t_fence = (t1 - t0) / ITER * 1000 * 1000;   /*us */

    if (rank == 0)
        fprintf(stdout, "%s%s, nproc %d config %s fence %lf\n",
#ifdef ENABLE_CSP
                "csp-", async_sched_level_str,
#else
                "original", "",
#endif
                nprocs, config_val == NULL ? "none" : config_val, t_fence);
